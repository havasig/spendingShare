...
class StatisticsPage extends StatelessWidget {
  const StatisticsPage({Key? key, required this.firestore, required this.color}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: SpendingShareAppBar(titleText: 'statistics'.tr),
    body: Text('statistics page'),
...
/*
    return Scaffold(
      ...
      body: Padding(
        ...
        child: StreamBuilder(
            stream: firestore.collection('members').where('userFirebaseId', isEqualTo: user.userFirebaseId).snapshots(),
            builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> transactionListSnapshot) {
              if (transactionListSnapshot.connectionState == ConnectionState.active ||
                ...
                if (transactionListSnapshot.hasData) {
                  Map<String, List<spending_share_transaction.Transaction>> groupTransactions;
                  for(var memberData in transactionListSnapshot.data!.docs) {
                    int t = await firestore.collection('groups').where('members', arrayContains: memberData).get();
                  }
